...
/*
	...
	By Jeremy
*/
public class EduTouch_drag_rotate_scale_control_ui : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (Input.touchCount == 1)
        {
            ...
            if (this.is_dragable == true)
            {
                ...
                else if (is_dragging && touch.phase == TouchPhase.Moved)
                {
                    ...
                    float speedModifierPosition = 1.0f;
                    this.GetComponent<RectTransform>().position = new Vector2(this.GetComponent<RectTransform>().position.x + touch.deltaPosition.x * speedModifierPosition,
                        this.GetComponent<RectTransform>().position.y + touch.deltaPosition.y * speedModifierPosition);
                }
                #endregion
        ...
        {
            if (is_scale == true && Input.touchCount == 2)
            {
                //没有长按拖动，计时器归零
                this.is_dragging = false;
                this.is_long_touch_timing = false;
